...
from geoboundaries import get_data, generate_dataset
...
def main():
    ...
    with Download() as downloader:
        ...
        for info, country in progress_storing_tempdir(lookup, countries, 'iso3'):
            ...
            dataset, resource_names = generate_dataset(countryiso3, country_admin_boundaries)
            if dataset:
                dataset.update_from_yaml()
                dataset.create_in_hdx(remove_additional_resources=True, hxl_update=False,
                                      updated_by_script='GeoBoundaries', batch=info['batch'])
